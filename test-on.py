...
def lightUpStayOn(color, location, duration):
    led.set(location, color)
    led.update()
    time.sleep(duration) #how long until the next event
...
for i in range(9,12): #number of month numbers in the document
    ...
    # generate all 160 slots
    r = range(PINCOUNT)
    random.shuffle(r)
    # now we assign location and pop off the element from the list.
    ...
    for event in themonth:
        ...
        if (x <= 160):
            ...
            location = r.pop() # take the last one from the list
            duration = time_per_death_event
           
            lightUpStayOn(color, location, time_per_death_event)
